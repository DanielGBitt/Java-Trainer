...
export default function CourseStudyPage() {
  ...
  const allRels = useMemo(() => [...relationships, ...basesDeDatosRelationships], []);

  const related = useMemo(() => {
    const allRelated = allRels.filter((r) => unitIds.includes(r.sourceId) || unitIds.includes(r.targetId));
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 sm:p-6 space-y-6">
      <div>
        <Link href={`/c/${courseId}/map`} className="text-sm text-primary hover:text-primary/80 mb-2 inline-block">
          ← Volver al Mapa
        </Link>
            ...
            <div className="flex flex-wrap gap-2">
              {(() => {
                ...
                related.forEach((r) => {
                  ...
                  const allNodes = buildGameMap(courseId);
                  const targetNode = allNodes.find((n) => n.knowledgeUnitIds.includes(relatedUnitId));
                  const href = targetNode ? `/c/${courseId}/study/${targetNode.id}` : "#";
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
        <Link href={`/c/${courseId}/practice/${nodeId}`} className="flex-1">
          <Button className="w-full" size="lg">
            Practicar
          </Button>
        </Link>
        <Link href={`/c/${courseId}/map`} className="flex-1">
